...
internal unsafe class TextureLoadManager : MonoBehaviour
{
    ...
    LoadHandle DoLoadTextureAsync(string path, bool linear, bool unreadable = true)
    {
        ...
        try
        {
            ...
            string filePath = Path.Combine(KSPUtil.ApplicationRootPath, "GameData", path);
            InFlightLoad load;
    ...
    internal abstract class InFlightLoad : IDisposable
    {
        ...
        {
            using var sample = new Sample("InFlightLoad.MoveNext");
    ...
    class InFlightDDSLoad : InFlightLoad
    {
        ...
        public static InFlightDDSLoad Start(string path, bool linear, bool unreadable = true)
        {
            ...
            using (var s0 = new Sample("GetRawTextureData"))
            {
                rawData = inFlight.texture.GetRawTextureData<byte>();
            }

            var copyJob = new CopyJob { input = fileData, output = rawData };
        ...
        {
            using var sample = new Sample("TextureLoadManager.GetTextureMetadata");
    ...
    class InFlightPNGLoad : InFlightLoad
    {
        ...
        public static InFlightPNGLoad Start(string path, bool linear, bool unreadable)
        {
            ...
            return new InFlightPNGLoad { path = path, texture = tex };
        }
    ...
    {
        return CreateUninitializedTexture(
            data.width,
            data.height,
            data.format,
            data.mips,
    ...
    {
        using var sample = new Sample("TextureLoadManager.CreateUninitializedTexture");
